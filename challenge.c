...
// =========================================================
/*
 * sparse_multiply — zero dynamic allocation inside this function.
 * All buffers (values, col_indices, row_ptrs, y) are pre-allocated
 * by the caller with sufficient capacity.
 *
 * Phase 1: scan A row-by-row, extract non-zeros into CSR buffers.
 *   row_ptrs[i]     = index in values[] where row i begins
 *   row_ptrs[rows]  = total nnz  (sentinel)
 *   values[k]       = non-zero value
 *   col_indices[k]  = its column index
 *
 * Phase 2: compute y = A*x using the CSR data.
 *   y[i] = sum over k in [row_ptrs[i], row_ptrs[i+1]) of values[k]*x[col_indices[k]]
 */
void sparse_multiply(
